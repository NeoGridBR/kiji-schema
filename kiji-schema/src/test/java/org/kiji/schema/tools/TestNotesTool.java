...
public class TestNotesTool extends KijiClientTest {
  ...
  private int runTool(BaseTool tool, String...arguments) throws Exception {
    ...
    try {
      ...
      mToolOutputLines = mToolOutputStr.split(LSEP);
    }
  }
  ...
  public void testValidateFlags() throws Exception {
    ...
    assertEquals("--target URI must include one or zero columns." + LSEP, mToolOutputStr);

    assertFailure("--target=kiji://.env/default/table/info:name", "--do=fake");
    assertEquals("Invalid --do command: 'fake'. Valid commands are set, remove, get" + LSEP,
        mToolOutputStr);

    assertFailure("--target=kiji://.env/default/table/info:name", "--do=get", "--in-family=info",
        "--key=key");
    assertEquals("--in-family requires that no columns are specified in --target" + LSEP,
        mToolOutputStr);
  }
  ...
  public void testSetValidatFlags() throws Exception {
    ...
    assertEquals("Cannot specify --prefix, --partial, or --regex while --do=set" + LSEP,
        mToolOutputStr);
    ...
    assertEquals("Cannot specify a family with --in-family while --do=set" + LSEP,
        mToolOutputStr);

    assertFailure("--target=" + getInfoNameURI(), "--do=set");
    assertEquals("--do=set requires --key and --value OR --map" + LSEP, mToolOutputStr);

    assertFailure("--target=" + getInfoNameURI(), "--do=set", "--key=key", "--map=bope");
    assertEquals("--map is mutually exclusive with --key and --value" + LSEP, mToolOutputStr);
  }
  ...
  public void testRemoveValidateFlags() throws Exception {
    ...
    assertEquals("Cannot specify more than one of --prefix, --partial, and --regex" + LSEP,
        mToolOutputStr);

    assertFailure("--target=" + getInfoNameURI(), "--do=remove", "--map=bope");
    assertEquals("Cannot specify --map while --do=remove" + LSEP, mToolOutputStr);

    assertFailure("--target=" + getInfoNameURI(), "--do=remove", "--value=bope");
    assertEquals("Cannot specify --value while --do=remove" + LSEP, mToolOutputStr);
  }
  ...
  public void testGetValidateFlags() throws Exception {
    ...
    assertEquals("Cannot specify --map while --do=get" + LSEP, mToolOutputStr);

    assertFailure("--target=" + getInfoNameURI(), "--do=get", "--value=bope");
    assertEquals("Cannot specify --value while --do=get" + LSEP, mToolOutputStr);
  }
